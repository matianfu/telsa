...
class HandshakeState extends State {
  ...
  sendHandshakeMessage (type, data) {
    ...
    this.hs.push(data)
    this.ctx.sendMessage(ContentType.HANDSHAKE, data)
  }
}
...
class ServerHelloDone extends HandshakeState {
  onServerHelloDone (data) {
    ...
    this.sendHandshakeMessage(HandshakeType.CERTIFICATE, Prepend24(concat([
      ...this.ctx.opts.clientCertificates.map(c => Prepend24(c))])))
    this.sendHandshakeMessage(HandshakeType.CLIENT_KEY_EXCHANGE, Prepend16(publicEncrypt({
...
class CertificateVerify extends HandshakeState {
  enter () {
    ...
    if (typeof key === 'function') {
      ...
      this.ctx.changeCipherSpec(clientWriteKey, clientWriteMacKey, iv)
      // send finished
...
class ChangeCipherSpec extends HandshakeState {
  onChangeCipherSpec () {
    this.ctx.serverChangeCipherSpec(this.hs.serverWriteKey, this.hs.serverWriteMacKey)
...
class Established extends State {
  ...
  // TODO
  write (data) {
    this.ctx.sendMessage(ContentType.APPLICATION_DATA, data)
  }
}
...
class Transport extends EventEmitter {
  ...
  */
  sendMessage (type, data) {
    ...
    this.outgoing.push(concat([UInt8(type), VER12, Prepend16(data)]))
    this.emit('outgoing')
  }
