...
const randomBuffer = size => crypto.randomFillSync(Buffer.alloc(size))
...
class State {
  ...
  static _setState (prev, next, ...args) {
    let ctx
    if (prev instanceof State) {
      ctx = prev.ctx
      let p
      for (p = Object.getPrototypeOf(prev);
        !(NextState.prototype instanceof p.constructor);
        p.hasOwnProperty('exit') && p.exit.apply(prev),
  ...
  // setState -> State._setState(this, NextState, ...args)
...
class HandshakeState extends State {
  constructor (ctx) {
    ...
    if (ctx instanceof HandshakeState) {
      ...
      // TODO to support resumable some properties should be store in ctx
      this.hs = {
        buffer: [],
        sessionId: 0,
