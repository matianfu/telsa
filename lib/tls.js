...
const VER12 = Buffer.from([0x03, 0x03])
...
const createCipher = (key, macKey, counter) => {
  ...
  return (type, data) => {
    ...
    let tbs = Buffer.concat([SN(), UInt8(type), VER12, Prepend16(data)])
    ...
    return Buffer.concat([
      iv, 
      c.update(Buffer.concat([data, mac, pad])), 
      c.final()
    ])
  }
}
...
const createDecipher = (key, macKey) => {
  ...
  return (type, data) => {
    ...
    let dec = Buffer.concat([d.update(data.slice(16)), d.final()])
...
class RecordLayer {
  ...
  write (type, data, callback) {
    ...
    let record = Buffer.concat([UInt8(type), VER12, Prepend16(data)])
...
class HandshakeContext {
  constructor () {
    ...
    this.preMasterSecret = Buffer.concat([VER12, randomBuffer(46)])
...
class ServerHello extends HandshakeState {
  enter () {
    this.write(HandshakeType.CLIENT_HELLO, Buffer.concat([
      VER12,
...
class ServerHelloDone extends HandshakeState {
  handleServerHelloDone (data) {
    ...
    this.write(HandshakeType.CERTIFICATE, Prepend24(Buffer.concat([
...
class CertificateVerify extends HandshakeState {
  ...
  handshakeMessagesSigned (algorithm, signature) {
    this.write(HandshakeType.CERTIFICATE_VERIFY, 
      Buffer.concat([algorithm, Prepend16(signature)]))
...
class Context extends Duplex {
  constructor (socket, opts) {
    ...
    try {
      ...
      State.prototype.setState.apply(this, [ServerHello])
    } catch (e) {
      console.log(e)
    }
  }
  ...
  */
  ...
  static createConnection (opts, callback) {
    ...
    socket.once('connect', () => {
      ...
      const tls = new Context(socket, opts)
      tls.once('error', err => {
