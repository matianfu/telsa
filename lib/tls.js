...
class InitState extends State {
  connect (port, host) {
    this.setState(SocketConnecting, port, host)
  }
}

/** socket connecting */
class SocketConnecting extends State {
...
/** socket connected */
class SocketConnected extends State {
  constructor (soc, socket) {
    super(soc)
    this.rp = soc instanceof SocketConnected ? soc.rp : new RecordProtocol(socket)
    this.rp.state = this
  }
...
class RecordProtocol {
  ...
  onFragment (type, frag) {
    const shift = size =>
      K(this.frag.slice(0, size))(this.frag = this.frag.slice(size))
...
*/
class HandshakeState extends SocketConnected {
...
*/
class Established extends SocketConnected {
  enter () {
...
Error handling policies depends on context.

In _write code block, or there are pending _write's callback, the error should be returned using callback.
Otherwise, the error can be emitted.
...
class Context extends Duplex {
  ...
  */
  _write (chunk, encoding, callback) {
    if (this.connected) {
      ...
      callback(new Error('pending write exists'))
    } else {
  ...
  */
  ...
  /**
  implement duplex _destroy
  */
  _destroy (err, callback) {
    this.state._destroy (err, callback)
  }
